...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Cat cat = new Cat("Pushok", "siberian", 4);
        Dog dog = new Dog("Bobik", "labrador", 5);
        Horse horse = new Horse("Zvezda", "Persheron", 6);
        Bird bird = new Bird("Kesha", "Ara", 15);

        Animal[] animals = {cat, dog, horse, bird};
        for (int i = 0; i < animals.length; i++) {
            animals[i].run(50);
            animals[i].jump((float) 0.1);
            if (animals[i] instanceof Dog) {
                ((Dog) animals[i]).swim(5);
            }
            if (animals[i] instanceof Horse) {
                ((Horse) animals[i]).swim(50);
            }
        }
    }
}
